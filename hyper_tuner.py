...
def tune_keras_model(model_name, folder, build_fn, preprocess_fn):
    ...
    train_ds = train_ds_raw.take(20) # Use ~320 images for tuning
    val_ds = val_ds_raw.take(10) # Use ~160 images for tuning
    ...
    for lr, bs in itertools.product(lrs, batch_sizes):
        ...
        history = model.fit(train_ds_tuned, epochs=TUNING_EPOCHS, validation_data=val_ds_tuned, verbose=1)
...
def main():
    # 0. Pre-flight Resilience
    metric_utils.setup_gpu()
    
    # ResNet50
    sys.path.insert(0, os.path.join(os.path.dirname(os.getcwd()), 'DL - imagenet'))
    try:
        if 'train_local' in sys.modules: del sys.modules['train_local']
        import train_local as resnet_train
        from tensorflow.keras.applications.resnet50 import preprocess_input as resnet_pre
    except ImportError:
        resnet_train, resnet_pre = None, None
    
    # MobileNet
    sys.path.insert(0, os.path.join(os.path.dirname(os.getcwd()), 'DL - mobilenet'))
    try:
        if 'train_local' in sys.modules: del sys.modules['train_local']
        ...
        mobilenet_train, mobilenet_pre = None, None
    
    # EfficientNet
    sys.path.insert(0, os.path.join(os.path.dirname(os.getcwd()), 'DL - efficientnet b0'))
    try:
        if 'train_local' in sys.modules: del sys.modules['train_local']
        import train_local as effnet_train
        from tensorflow.keras.applications.efficientnet import preprocess_input as effnet_pre
    except ImportError:
        effnet_train, effnet_pre = None, None
